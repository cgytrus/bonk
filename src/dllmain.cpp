...
namespace bonk {
    ...
    matdash::cc::thiscall<bool>
    PlayerObject_collidedWithObject(gd::PlayerObject *self, gd::GameObject *obj, int a, int b, int c, int d) {
        _deathCause = obj;
    ...
    matdash::cc::thiscall<void>
    PlayLayer_destroyPlayer(gd::PlayLayer *self, gd::PlayerObject *ply, gd::GameObject *obj) {
        if (obj != nullptr)
            _deathCause = obj;
...
void mod_main(HMODULE) {
    bonk::config::loadConfig();
    bonk::config::initMegahack();
    if(bonk::config::enabled)
        bonk::config::enableHooks();
}
